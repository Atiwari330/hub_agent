...
export async function fetchSupportPulseData(): Promise<SupportPulseResponse> {
  ...
  if (openError) {
    throw new Error(`Failed to fetch tickets: ${openError.message}`);
  }
  ...
  const { data: closedTickets, error: closedError } = await supabase
    .from('support_tickets')
    .select('hs_primary_company_id, time_to_close')
    .eq('is_closed', true)
    .gte('closed_date', ninetyDaysAgo.toISOString());

  if (closedError) {
    throw new Error(`Failed to fetch closed tickets: ${closedError.message}`);
  }
