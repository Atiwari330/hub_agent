...
import { analyzeRcmTicket } from '../analyze/analyze-rcm-core';
...
/**
 * POST /api/queues/rcm-audit/batch-analyze
 *
 * Batch analyze multiple tickets for RCM relevance with
...
export async function POST(request: NextRequest) {
  ...
  try {
          ...
          try {
            ...
            if (result.success) {
              const event: ProgressEvent = {
                ...
                status: 'success',
                analysis: {
                  is_rcm_related: result.analysis.is_rcm_related,
                  rcm_system: result.analysis.rcm_system,
                  issue_category: result.analysis.issue_category,
                  severity: result.analysis.severity,
                  current_status: result.analysis.current_status,
                  issue_summary: result.analysis.issue_summary,
                  linear_assessment: result.analysis.linear_assessment,
    ...
    console.error('Batch RCM audit error:', error);
    return new Response(
      JSON.stringify({
