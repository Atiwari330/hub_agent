...
/**
 * POST /api/queues/support-trainer/batch-analyze
 *
...
export async function POST(request: NextRequest) {
  ...
  try {
          ...
          try {
            ...
            if (result.success) {
              const event: ProgressEvent = {
                ...
                status: 'success',
                analysis: {
                  customer_ask: result.analysis.customer_ask,
                  problem_breakdown: result.analysis.problem_breakdown,
                  system_explanation: result.analysis.system_explanation,
                  interaction_timeline: result.analysis.interaction_timeline,
                  resolution_approach: result.analysis.resolution_approach,
                  coaching_tips: result.analysis.coaching_tips,
                  knowledge_areas: result.analysis.knowledge_areas,
                  difficulty_level: result.analysis.difficulty_level,
    ...
    console.error('Batch support trainer analysis error:', error);
    return new Response(
      JSON.stringify({
