...
interface ActiveDealWithMetadata {
  id: string;
...
interface ProcessedDeal extends ActiveDealWithMetadata {
  severity: StalledSeverity;
}

type SortColumn = 'dealName' | 'ownerName' | 'amount' | 'stageName' | 'daysSinceActivity';
...
const PRESETS: Record<Exclude<PresetName, 'custom'>, ThresholdConfig> = {
  ...
  default: {
    minInactiveDays: STALLED_THRESHOLDS.MIN_INACTIVE,
    watchThreshold: STALLED_THRESHOLDS.WATCH,
    warningThreshold: STALLED_THRESHOLDS.WARNING,
    criticalThreshold: STALLED_THRESHOLDS.CRITICAL,
    minDealAge: STALLED_THRESHOLDS.MIN_AGE,
  },
  lenient: {
    minInactiveDays: 14,
    watchThreshold: 14,
...
function detectPreset(config: ThresholdConfig): PresetName {
  for (const [name, preset] of Object.entries(PRESETS) as [Exclude<PresetName, 'custom'>, ThresholdConfig][]) {
    if (
      config.minInactiveDays === preset.minInactiveDays &&
      ...
      config.minDealAge === preset.minDealAge
    ) {
      return name;
    }
  }
...
function computeSeverity(daysSinceActivity: number, thresholds: ThresholdConfig): StalledSeverity {
  if (daysSinceActivity > thresholds.criticalThreshold) return 'critical';
  if (daysSinceActivity > thresholds.warningThreshold) return 'warning';
  return 'watch';
}
...
function getEffectiveAnalysis(
  ...
  // Fresh analysis from client-triggered re-analysis takes priority
  if (freshAnalyses[deal.id]) {
    return freshAnalyses[deal.id];
  }
  // Fall back to cached analysis from the API
...
function getAiDotColor(analysis: AnalysisData | null): string | null {
  ...
  // Check overdue for date statuses
...
function DiagnosisFactors({ deal }: { deal: ProcessedDeal }) {
  interface Factor {
    label: string;
  ...
  // Always show inactivity (primary reason)
  factors.push({
    label: `No activity for ${deal.daysSinceActivity} business days`,
    severity: deal.severity === 'critical' ? 'critical' : deal.severity === 'warning' ? 'warning' : 'info',
  });

  // No future activity
  if (!deal.hasFutureActivity) {
    factors.push({ label: 'No future activity scheduled', severity: 'warning' });
  }
  ...
  // Next step overdue
  if (deal.nextStepOverdue) {
    factors.push({ label: 'Next step is overdue', severity: 'critical' });
  }
...
export function StalledUpsellsQueueView() {
  // Raw API data
  const [allDeals, setAllDeals] = useState<ActiveDealWithMetadata[]>([]);
  ...
  const [futureActivityFilter, setFutureActivityFilter] = useState<BooleanFilter>('all');
  ...
  // Extract unique AEs from ALL deals (not just stalled ones)
  ...
  // Extract unique stages from ALL deals
  ...
  // Client-side staleness detection + filtering + sorting
  const processedDeals = useMemo(() => {
    ...
    // Step 1: Apply staleness thresholds to determine which deals are stalled
    const stalledDeals: ProcessedDeal[] = [];

    for (const deal of quarterFiltered) {
      // Must be old enough
      if (deal.dealAgeDays <= thresholds.minDealAge) continue;

      // Must be inactive enough
      if (deal.daysSinceActivity <= thresholds.minInactiveDays) continue;

      // Must have no future activity scheduled
      if (deal.hasFutureActivity) continue;

      const severity = computeSeverity(deal.daysSinceActivity, thresholds);
      stalledDeals.push({ ...deal, severity });
    }

    // Step 2: Apply filters
    let result = stalledDeals;
    ...
    if (futureActivityFilter !== 'all') {
      result = result.filter((d) => (futureActivityFilter === 'yes') === d.hasFutureActivity);
    }
    ...
    return result;
  }, [allDeals, thresholds, quarterFilter, currentYear, aeFilter, stageFilter, closeDateFilter, amountMin, amountMax, nextStepFilter, futureActivityFilter, sortColumn, sortDirection]);
  ...
  const fetchData = useCallback(async () => {
    try {
      setLoading(true);
      const response = await fetch('/api/queues/stalled-upsells');
      if (!response.ok) {
  ...
  return (
    <div className="p-6">
      {/* Header */}
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-gray-900">Stalled Upsells</h1>
        <p className="text-sm text-gray-600 mt-1">
          Active upsell deals with no recent activity and no future activity scheduled.
        </p>
      </div>
      ...
      {thresholdsOpen && (
        <div className="bg-gray-50 border border-gray-200 rounded-lg p-4 mb-4">
          <div className="flex flex-wrap items-center gap-4">
            <NumberInput
              label="Min Inactive Days"
              value={thresholds.minInactiveDays}
              onChange={(v) => updateThreshold('minInactiveDays', v)}
            />
            <NumberInput
              label="Min Deal Age"
              value={thresholds.minDealAge}
              onChange={(v) => updateThreshold('minDealAge', v)}
            />
      ...
      <div className="flex flex-wrap items-center gap-3 mb-4">
        ...
        {/* Future Activity */}
        <div className="flex items-center gap-2">
          <label className="text-sm text-gray-600">Future Activity:</label>
          <select
            value={futureActivityFilter}
            onChange={(e) => setFutureActivityFilter(e.target.value as BooleanFilter)}
            className="text-sm border border-gray-300 rounded-lg px-3 py-1.5 bg-white focus:outline-none focus:ring-2 focus:ring-indigo-500"
          >
            <option value="all">All</option>
            <option value="yes">Yes</option>
      ...
      {!loading && !error && processedDeals.length === 0 && (
        <div className="text-center py-12 bg-gray-50 rounded-lg">
          ...
          <h3 className="mt-4 text-lg font-medium text-gray-900">No stalled upsells</h3>
          <p className="mt-1 text-sm text-gray-500">
            {hasActiveFilters
              ? 'No deals match the current filters. Try adjusting the thresholds or filters.'
              : 'All active upsell deals have recent activity or future activities scheduled.'}
          </p>
        </div>
      )}
                ...
                <tr className="bg-slate-50 border-b border-gray-200">
                  ...
                  >
                    <div className="flex items-center gap-1">
                      ...
                      <SortIcon active={sortColumn === 'daysSinceActivity'} direction={sortDirection} />
                    </div>
                  </th>
