...
export function StalledDealsQueueView() {
  // Raw API data
  ...
  const fetchData = useCallback(async () => {
    try {
      setLoading(true);
      const response = await fetch('/api/queues/stalled-deals');
      if (!response.ok) {
  ...
  return (
    <div className="p-6">
      {/* Header */}
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-gray-900">Stalled Deals</h1>
        <p className="text-sm text-gray-600 mt-1">
          Active deals with no recent activity and no future activity scheduled.
        </p>
      </div>
      ...
      {!loading && !error && processedDeals.length === 0 && (
        <div className="text-center py-12 bg-gray-50 rounded-lg">
          ...
          <h3 className="mt-4 text-lg font-medium text-gray-900">No stalled deals</h3>
          <p className="mt-1 text-sm text-gray-500">
            {hasActiveFilters
              ? 'No deals match the current filters. Try adjusting the thresholds or filters.'
              : 'All active deals have recent activity or future activities scheduled.'}
          </p>
        </div>
      )}
