...
export function Sidebar({ owners, lastSync, quarterLabel, quarterProgress, onCollapsedChange, user, isStale, syncHealth }: SidebarProps) {
  ...
  // Pricing Compliance
  const isOnPricingCompliance = pathname === '/dashboard/pricing-compliance';
  // Admin
...
{/* Hot Tracker Link */}
        ...
        {/* Pricing Compliance Dashboard */}
        {hasPermission(user, RESOURCES.PRICING_COMPLIANCE) && (
          <div className="mb-2">
            <Link
              href="/dashboard/pricing-compliance"
              className={`flex items-center gap-3 py-2.5 text-sm font-medium transition-colors ${isCollapsed ? 'px-0 justify-center' : 'px-4'} ${
                isOnPricingCompliance
                  ? 'bg-indigo-600 text-white'
                  : 'text-slate-300 hover:bg-slate-800'
              }`}
              title={isCollapsed ? 'Pricing Compliance' : undefined}
            >
              <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" />
              </svg>
              {!isCollapsed && <span>Pricing Compliance</span>}
            </Link>
          </div>
        )}
